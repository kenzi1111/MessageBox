...
def delete(message_id):
    if (
        Message.select()
        .where((Message.id == message_id) & (Message.user == current_user))
        .first()
    ):
...
def reply(message_id):
    Message.create(
        user=current_user, content=request.form["content"], reply_to=message_id
    )
...
def show(message_id):
    ...
    messages = (
        Message.select()
        .where((Message.reply_to == message_id))
        .order_by(Message.pub_date.desc(), Message.id.desc())
    )
    return render_template(
        "show.html", message=message, messages=messages, message_id=message_id
    )
